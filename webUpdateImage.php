...
<html>
	...
	<head>
		...
		<script type='text/javascript' src='./js/ajxUpdateImage.js'></script>
		<script type='text/javascript' src='./js/ajxDeleteImage.js'></script>
	...
	<body>
		...
		<div class='wrapper'>
			...
			<section>
				<?php
					echo "<input class='idImage' type='hidden' name='idImage' value='" . $idImage . "' />";
					// Open DB
					include_once("dbConfig.php");
					$mysqli = new mysqli(DB_HOST, DB_LOGIN, DB_PWD, DB_NAME);
					$mysqli->set_charset("utf8");
					// Query : select
					$query = "SELECT title, size, borderColor FROM tblImages WHERE id = $idImage;";
					$result = $mysqli->query($query);
					// Query result
					$row = $result->fetch_assoc();
					echo "<input class='title'type='text' name='title' placeholder=\"Titre de l'image\" autofocus value=\"" .  $row['title'] . "\"/>";
