$(document).ready(function(){
    ...
    function drawTabs(id, title, posX, posY, tblDrawings, borderColor, size){
        var delayTime = 0;
        newDrawing(id, title, posX, posY, tblDrawings, borderColor, size);
        for (var j=0; j < tblDrawings.length; j++){
            delayTime += 0;
            if(j != 0)
                delayTime += (50*tblDrawings[j-1].tblPoints.length + 100 * tblDrawings[j-1].delay);
            setTimeout(drawLines, delayTime, id, j, tblDrawings[j].color, tblDrawings[j].width, tblDrawings[j].tblPoints, posX, posY, size);
        }
    }

    function drawLines(id, idLine, color, width, tblPoints, posX, posY, size){
        var obj = {
            layer: true,
            strokeStyle: color,
            strokeWidth: width,
    ...
    function newDrawing(id, title, posX, posY, tblRange, borderColor, size) {
        posX -= 0;
        posY -= 0;
        jQuery('#mainCanvas')
        //Draws the Icon
        ...
        for (var val of data) {
            ...
            if(defined(val.idDrawing) && defined(val.posX) && defined(val.title) && defined(val.posY)){
                ...
                //If it never existed create the Drawing
                    ...
                    drawTabs(val.idDrawing, val.title, val.posX, val.posY, val.tblLines, val.borderColor, val.size);
